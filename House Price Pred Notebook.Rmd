...
# Creating Recipe
recipe_house <- recipe(SalePrice  ~ ., data = train) %>%
  step_rm(Id,
          X1,
          GarageYrBlt) %>%
...
final_lasso <- finalize_workflow(wf_lr, best_rmse_lr)
...
```{r}
lr_final <- last_fit(final_lasso, df_split)
